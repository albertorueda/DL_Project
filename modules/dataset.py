...
class AISDataset(Dataset):
    def __init__(self, df: pd.DataFrame, seq_input_length: int = 6, seq_output_length: int = 1):
        ...
        for i in range(len(df) - seq_input_length - seq_output_length):
            mmsi_start = df['MMSI'].iloc[i]
            mmsi_in = df['MMSI'].iloc[i:i+seq_input_length]
            mmsi_out = df['MMSI'].iloc[i+seq_input_length : i+seq_input_length+seq_output_length]

            if (mmsi_in.eq(mmsi_start).all()) and (mmsi_out.eq(mmsi_start).all()):
    ...
    def __getitem__(self, idx):
        ...
        x = self.dataframe.iloc[real_idx: real_idx + self.seq_input_length]
        y = self.dataframe.iloc[real_idx + self.seq_input_length: real_idx + self.seq_input_length + self.seq_output_length]
        return x, y
